...
CPeripheralCecFrameworkAdapter::CPeripheralCecFrameworkAdapter(
		CPeripherals& manager, const PeripheralScanResult& scanResult,
		CPeripheralBus* bus) :
		...
		const char *sender, const char *message, const CVariant &data)
{
			...
			&& !strcmp(message, "OnScreensaverDeactivated") && m_bIsReady)
	{
		...
		if (data["shuttingdown"].isBoolean())
		{
			...
			if (bIgnoreDeactivate)
				CLog::Log(LOGDEBUG,
						"%s - ignoring OnScreensaverDeactivated for power action",
						__FUNCTION__);
		}
	...
	{
		CLog::Log(LOGDEBUG,
				"%s - reconnecting to the CEC adapter after standby mode",
				__FUNCTION__);
		if (ReopenConnection())
		{
...
bool CPeripheralCecFrameworkAdapter::OpenConnection(void)
{
	bool bIsOpen(false);

	if (!GetSettingBool("enabled"))
	{
		CLog::Log(LOGDEBUG, "%s - CEC adapter is disabled in peripheral settings",
				__FUNCTION__);
		m_bStarted = false;
		return bIsOpen;
	}
	...
	CLog::Log(LOGDEBUG, "%s - opening a connection to the CEC adapter: %s",
			__FUNCTION__, m_strComPort.c_str());
	...
	std::string strMessage = StringUtils::Format(
			g_localizeStrings.Get(21336).c_str(),
			g_localizeStrings.Get(36000).c_str());
	CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info,
			g_localizeStrings.Get(36000), strMessage);

	bool bConnectionFailedDisplayed(false);

	while (!m_bStop && !bIsOpen)
	{
		if ((bIsOpen = m_cecAdapter->Open(m_strComPort.c_str())) == false)
		{
			// display warning: couldn't initialise libCEC
			CLog::Log(LOGERROR,
					"%s - could not opening a connection to the CEC adapter",
					__FUNCTION__);
			if (!bConnectionFailedDisplayed)
				CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error,
						g_localizeStrings.Get(36000), g_localizeStrings.Get(36012));
	...
	if (bIsOpen)
	{
		CLog::Log(LOGDEBUG, "%s - connection to the CEC adapter opened",
				__FUNCTION__);
	}
...
void CPeripheralCecFrameworkAdapter::ProcessVolumeChange(void)
{
	...
	case VOLUME_CHANGE_UP:
		m_cecAdapter->SendKeypress(CECDEVICE_AUDIOSYSTEM,
	...
	case VOLUME_CHANGE_MUTE:
		m_cecAdapter->SendKeypress(CECDEVICE_AUDIOSYSTEM,
				CEC_USER_CONTROL_CODE_MUTE, false);
		{
			CSingleLock lock(m_critSection);
...
void CPeripheralCecFrameworkAdapter::SetMenuLanguage(const char *strLanguage)
{
	...
	if (!strGuiLanguage.empty())
	{
		...
		CApplicationMessenger::GetInstance().PostMsg(TMSG_SETLANGUAGE, -1, -1,
				nullptr, strGuiLanguage);
		CLog::Log(LOGDEBUG, "%s - language set to '%s'", __FUNCTION__,
				strGuiLanguage.c_str());
	}
	else
		CLog::Log(LOGWARNING, "%s - TV menu language set to unknown value '%s'",
				__FUNCTION__, strLanguage);
}
...
void CPeripheralCecFrameworkAdapter::OnTvStandby(void)
{
	...
	default:
		CLog::Log(LOGERROR,
				"%s - Unexpected [standby_pc_on_tv_standby] setting value",
				__FUNCTION__);
		break;
	}
}
...
{
	CLog::Log(LOGDEBUG, "%s - received key %2x duration %d", __FUNCTION__,
			key.iButton, key.iDuration);
...
{
public:
	CPeripheralCecFrameworkAdapterReopenJob(
			CPeripheralCecFrameworkAdapter *adapter) :
			m_adapter(adapter)
	{
	}
...
bool CPeripheralCecFrameworkAdapter::ReopenConnection(bool bAsync /* = false */)
{
	if (bAsync)
	{
		CJobManager::GetInstance().AddJob(
				new CPeripheralCecFrameworkAdapterReopenJob(this), nullptr,
...
bool CPeripheralCecFrameworkAdapter::ToggleDeviceState(
		CecStateChange mode /*= STATE_SWITCH_TOGGLE */, bool forceType /*= false */)
{
	if (!IsRunning())
		return false;
	if (IsActiveSource()
		...
		__u8 reason)
{
	...
	if (reason == CEC_OP_ABORT_UNRECOGNIZED_OP)
	{
		...
		if (m_laInfo[la].feature_aborted[opcode].count == 2)
		{
			...
			warn(
					"replying Feature Abort [Unrecognized Opcode] to the same message.\n");
		}
	}
	else if (m_laInfo[la].feature_aborted[opcode].count)
	{
		warn(
				"Replying Feature Abort with abort reason different than [Unrecognized Opcode]\n");
		warn(
				"to message that has previously been replied Feature Abort to with [Unrecognized Opcode].\n");
	}
	else
...
void CPeripheralCecFrameworkAdapter::rcPressHoldStop(const PState state)
{
	...
	dev_info(
			"Stop Press and Hold. Mean duration between User Control Pressed messages: %dms\n",
			mean_duration);
		...
		unsigned me)
{
	...
	case CEC_MSG_USER_CONTROL_PRESSED:
	{
		...
		if (m_node->state.rc_state == NOPRESS)
			dev_info("Button press: %s\n", getUiCmdString(rc_press.ui_cmd));
		else if (rc_press.ui_cmd != m_node->state.rc_ui_cmd)
		{
			...
			if (duration > MAX_INITIATOR_REP_TIME)
				warn(
						"Device waited more than the maximum Initiatior Repetition Time and should have sent a User Control Released message.");
		}
	...
	case CEC_MSG_USER_CONTROL_RELEASED:
		...
		if (m_node->state.rc_state == NOPRESS)
			dev_info("Button release (unexpected, %ums after last press)\n",
					tsToMs(msg.rx_ts - m_node->state.rc_press_rx_ts));
		else if (getDurationMs(msg.rx_ts,
				m_node->state.rc_press_rx_ts) > FOLLOWER_SAFETY_TIMEOUT)
			dev_info("Button release: %s (after timeout, %ums after last press)\n",
					getUiCmdString(m_node->state.rc_ui_cmd),
					tsToMs(msg.rx_ts - m_node->state.rc_press_rx_ts));
		else
	...
	case CEC_MSG_SYSTEM_AUDIO_MODE_REQUEST:
	{
		...
		dev_info("System Audio Mode: %s\n",
				m_node->state.sac_active ? "on" : "off");
		return;
	}
	...
	while (1)
	{
		...
		if (FD_ISSET(fd, &rd_fds))
		{
					...
					&& tsToMs(getTs() - m_laInfo[from].feature_aborted[opcode].ts) < 200)
			{
				...
				warn(
						"replying Feature Abort (not [Unrecognized Opcode]) to the same message.\n");
			}
			if (from != CEC_LOG_ADDR_UNREGISTERED && !m_laInfo[from].ts)
				dev_info("Logical address %d (%s) discovered.\n", from, la2s(from));
			if (m_showMsgs)
			{
				printf("    %s to %s (%d to %d): ", la2s(from),
						to == 0xf ? "all" : la2s(to), from, to);
				if (m_showInfo)
					printf("\tSequence: %u Rx Timestamp: %llu.%03llus\n", msg.sequence,
							msg.rx_ts / 1000000000, (msg.rx_ts % 1000000000) / 1000000);
			}
